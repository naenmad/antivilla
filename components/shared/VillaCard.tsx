    ...
    return (
        <Card className="overflow-hidden group border-0 shadow-sm hover:shadow-xl transition-all duration-300 rounded-2xl bg-white ring-1 ring-slate-100">
            <div className="relative aspect-[4/3] overflow-hidden">
                ...
                {/* Anti-Zonk Badge */}
                {villa.isVerified && (
                    <div className="absolute top-3 left-3 bg-white/95 backdrop-blur-sm px-2.5 py-1 rounded-full flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wide text-emerald-800 shadow-sm z-10">
            ...
            <CardContent className="p-5">
                <div className="flex justify-between items-start mb-3 gap-2">
                    <div className="flex-1 min-w-0">
                        <Link href={`/villas/${villa.slug}`} className="group-hover:text-emerald-700 transition-colors block">
                ...
                <div className="flex flex-wrap gap-2 mt-4">
                    ...
                    {villa.view.slice(0, 1).map((v) => (
                        <Badge key={v} variant="secondary" className="text-xs bg-slate-100 text-slate-600 hover:bg-slate-200 border-0">
                            {v}
                        </Badge>
                    ))}
                    {villa.view.length > 1 && (
            ...
            <CardFooter className="p-5 pt-0 flex items-end justify-between border-t border-slate-50 mt-1">
